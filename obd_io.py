...
def decrypt_dtc_code(code):
    ...
    for i in range(0, 3):
        ...
        dtc.append("{}{}{}{}{}".format(type, dig1, dig2, dig3, dig4))
        current = current[4:]
    return dtc
...
class OBDPort:
    ...
    def __init__(self, portnum, _notify_window, SERTIMEOUT, RECONNATTEMPTS):
        ...
        debug_display(
            self._notify_window,
            1,
            "Interface successfully {} opened".format(self.port.portstr)
        )
        debug_display(
            self._notify_window,
            1,
    ...
    def log(self, sensor_index, filename):
        ...
        if file:
            ...
            while True:
                now = datetime.now()
                data = self.sensor(sensor_index)
                line = "%.6f,\t%s\n".format((now - start_time).isoformat(), data[1])
                file.write(line)
                file.flush()
